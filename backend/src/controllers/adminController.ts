...
export const adminController = {
  ...
  consultarCatalogo: async (req: Request, res: Response) => {
    try {
      ...
      const [servicios, total, estadisticas, estadisticasGenerales] = await Promise.all([
        // Servicios con estadísticas detalladas
        ...
        prisma.$transaction([
          ...
          // Clientes activos (con suscripciones activas)
          prisma.cliente.count({
            where: {
              suscripciones: {
                some: {
  ...
  agregarServicio: async (req: Request, res: Response): Promise<Response | void> => {
    try {
      ...
      // Verificar que no existe un servicio con el mismo nombre
  ...
  actualizarServicio: async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const { id } = req.params;
      const adminId = (req.user as any)?.id;
      ...
      // Verificar permisos (solo el admin creador puede modificar)
  ...
  eliminarServicio: async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const { id } = req.params;
      const adminId = (req.user as any)?.id;
      ...
      // Verificar que el servicio existe y obtener información de suscripciones
      ...
      // Eliminar el servicio (esto también eliminará carritos items por CASCADE)
  ...
  consultarClientes: async (req: Request, res: Response) => {
    try {
      const { 
        activo, 
        page = '1', 
  ...
  suspenderCliente: async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const { id } = req.params;
      ...
      // Suspender cliente y pausar suscripciones activas
  ...
  consultarCarrito: async (req: Request, res: Response): Promise<Response | void> => {
    try {
      ...
      // Obtener carritos del cliente (activo y históricos)
